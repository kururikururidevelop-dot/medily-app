...
- フィールド（例）:
  ...
  - isResponder: boolean
  - categories: array of category ids (専門領域)
...
- スキーマ（例）:
  ...
  - Gender/Age filters (optional) (性別・年齢フィルタ条件)
- 実装上の注意: `body` または `description` フィールドで質問本文を格納。API では `description` を使用する場合もある。統一を推奨。
