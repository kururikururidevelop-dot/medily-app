...
本書は本システム（LIFF + LINE DM + Firebase）を開発・運用するための処理方式設計をまとめる。実行環境（インフラ）、開発環境（ライブラリ）、UI/UX 実装方式、フロントエンド処理方式、バックエンド処理方式を網羅する。モックの作成に関する注意点も含む。
...
- フロントエンド（LIFF / PRサイト）
  ...
  - LIFF は HTTPS でホストされる必要があるためホスティングを利用（Vercel / Firebase Hosting）。
- バックエンド
  - Firebase
    ...
    - Firebase Auth（必要に応じて）：ユーザー管理（LIFF と紐付け）
...
- LINE / LIFF
  - `@line/liff`（LIFF SDK）
  - `@line/bot-sdk`（サーバー側での Messaging API 呼び出し）
- Firebase
  - `firebase`（クライアント SDK）
...
- 技術構成
  - Next.js（ページ／LIFF 向け）＋ React コンポーネント
  - 状態管理：`React Context`（認証・UI 状態） + `swr`/`react-query`（データフェッチ・キャッシュ）
  - UI ライブラリ：Tailwind ベースの独自コンポーネント（必要時に MUI の React コンポーネント）
- 認証・ユーザー識別
  - LIFF で取得した ID/プロフィールをクライアントで保持し、必要に応じて Firebase Auth カスタムトークンを利用して連携
- 質問投稿ワークフロー（例）
  1. ユーザーが LIFF 上で質問を入力 → フロントがバリデーション
  2. フロントは POST `/api/questions`（Next.js API または Functions）へ送信
  3. 成功時は画面遷移／トースト表示
- 回答表示ワークフロー
  - 回答は LINE DM 経由で受信・保存されるが、LIFF の質問詳細（L020）では Firestore を参照して回答一覧を取得して表示
...
- データフロー（主な処理）
  - 質問作成
    1. Frontend → API (`/api/questions`) → Functions が受け取り Firestore に保存
    2. Functions がマッチングロジックを実行、候補に LINE Push（`@line/bot-sdk`）で通知（Flex Message）
  - 回答受信
    1. LINE webhook (Functions) がメッセージ受信 → 検証（署名）→ 保存（answers コレクション）
    2. 保存トリガーで個別スレッドの集約・ステータス更新・貢献度更新を実行
    3. 回答者の貢献集計は Atomic なトランザクションで更新（Firestore トランザクション）
  - AI チェック
    - 回答受信前または受信時に LLM ベースのチェック（NGワード、診断行為、誹謗中傷）を実行
    - 実装方法：Functions 内で LLM API（外部 API or GCP Vertex AI）呼び出しを行い、違反時は保存をブロックまたはフラグ付け
  - 貢献度画像生成
    - 選択肢：
      - クライアント描画（SVG/CANVAS）→ 軽量で即時反映（L030）
...
- Frontend
  - `next`, `react`, `tailwindcss`, `@tailwindcss/forms`, `swr`/`@tanstack/react-query`, `@line/liff`
- Backend
